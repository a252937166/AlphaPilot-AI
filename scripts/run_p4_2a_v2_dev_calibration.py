...
def _load_preregistration(root: Path, design: Mapping[str, Any]) -> JsonObject:
    path = (root / ROUND_PREREGISTRATION_PATH).resolve()
    if (
        path.is_symlink()
        or not path.is_file()
        or _sha256_file(path) != ROUND_PREREGISTRATION_SHA256
    ):
        raise CalibrationRoundError("Round 1 preregistration bytes drifted")
    prereg = _load_json(path, "Round 1 preregistration")
    _exact_keys(
        prereg,
        {
            "schema_version",
    ...
    if design_ref.get("sha256") != DESIGN_SHA256:
        raise CalibrationRoundError("Round 1 design binding drifted")
    ...
    if (
        ...
        raise CalibrationRoundError("Round 1 development frame drifted")
    ...
    if (
        ...
        raise CalibrationRoundError("Round 1 execution contract drifted")
    ...
    if (
        ...
        raise CalibrationRoundError("Round 1 gate contract drifted")
...
def _load_contracts(root: Path, prereg: Mapping[str, Any]) -> tuple[ContractBinding, ...]:
    models = prereg.get("models")
    if not isinstance(models, list) or len(models) != 2:
        raise CalibrationRoundError("Round 1 must register exactly two models")
    ...
    if "[P4_NEWS_EVENT_EXTRACT v2-r1]" not in prompt:
        raise CalibrationRoundError("Round 1 prompt marker drifted")
    ...
    for expected_model, entry_value in zip(MODEL_ORDER, models, strict=True):
        ...
        if entry.get("model_slug") != expected_model or entry.get("model") != expected_model:
            raise CalibrationRoundError("Round 1 model order drifted")
        ...
        try:
            ...
            raise CalibrationRoundError("Round 1 model contract is invalid") from exc
        if not isinstance(value, dict):
            raise CalibrationRoundError("Round 1 model contract must be a mapping")
        ...
        if (
            wrapper.get("schema_version") != "p4.2a-development-event-extract-contract-v2-r1"
            or wrapper.get("round_number") != 1
            ...
            raise CalibrationRoundError("Round 1 wrapper contract identity drifted")
        ...
        if (
            ...
            raise CalibrationRoundError("Round 1 base contract binding drifted")
        ...
        if wrapper_prompt_path != prompt_path or wrapper_prompt_sha != prompt_sha:
            raise CalibrationRoundError("Round 1 prompt binding drifted")
        ...
        if (
            ...
            raise CalibrationRoundError("Round 1 result schema binding drifted")
        ...
        if (
            ...
            raise CalibrationRoundError("Round 1 LLM controls drifted")
        ...
        if dict(isolation) != {
            ...
            raise CalibrationRoundError("Round 1 isolation contract drifted")
        contract = replace(
            base,
            path=wrapper_path,
            sha256=wrapper_sha,
    ...
    if comparable[0] != comparable[1]:
        raise CalibrationRoundError("Round 1 model contracts differ beyond llm.model")
...
def _score(
    ...
    return {
        ...
        "both_materiality_gates_passed": technical_complete and precision_passed and for_passed,
    }
...
def _artifact_paths(root: Path, model_slug: str) -> Mapping[str, Path]:
    directory = root / "docs/phase4/eval/v2-calibration/development/rounds/r1" / model_slug
    return {
...
def run_round(
    *,
    ...
    """Run the one registered dev45 Round 1, always flash then plus, with zero retry."""

    root = project_root.resolve()
    active_clock = clock or (lambda: datetime.now(UTC))
    design = _load_design(root)
    prereg = _load_preregistration(root, design)
    contracts = _load_contracts(root, prereg)
    ...
    outcome_path = root / "docs/phase4/eval/v2-calibration/development/rounds/r1/round-outcome.json"
    state_path = root / "docs/phase4/eval/v2-calibration/development/calibration.state.jsonl"
    all_paths = [outcome_path, state_path]
    for binding in contracts:
        all_paths.extend(_artifact_paths(root, binding.model_slug).values())
    if any(path.exists() or path.is_symlink() for path in all_paths):
        raise CalibrationRoundError("Round 1 create-only output or state already exists")
    ...
    for binding in contracts:
        paths = _artifact_paths(root, binding.model_slug)
        ...
        try:
            ...
            prediction_rows = _load_jsonl(paths["predictions"], "Round 1 predictions")
            metrics = _score(prediction_rows, gold_rows, baseline_strata)
            manifest: JsonObject = {
    ...
    outcome: JsonObject = {
        ...
        "model_reports": {
            model: {
                "path": _artifact_paths(root, model)["report"].relative_to(root).as_posix(),
                "sha256": _sha256_file(_artifact_paths(root, model)["report"]),
    ...
    _append_terminal(
        ...
        expected_start="round_started",
    )
    result = RoundResult(outcome_path, state_path, selected, reports)
    if fatal is not None:
        raise CalibrationRoundError("Round 1 recorded a terminal technical failure") from fatal
    return result


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the one registered P4.2a v2 dev45 Round 1")
    parser.add_argument(
        "--round-preregistration",
        type=Path,
        default=ROUND_PREREGISTRATION_PATH,
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if arguments.round_preregistration != ROUND_PREREGISTRATION_PATH:
        print("ERROR: Round 1 preregistration path is fixed", file=sys.stderr)
        return 1
    try:
        result = run_round()
