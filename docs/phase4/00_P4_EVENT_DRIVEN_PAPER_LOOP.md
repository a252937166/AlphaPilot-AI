...
3. **P4.1 验收判据不变**（数据完整性：去重、PIT、巨潮逐日 >0），本缺口不追溯改判；
   但下列为 **P4.3/P4.4 上线的新增前置门**（其数据尚未产生，此刻登记合规）：
   ① v2 修复 `akshare_ths` 重试预算缺陷；② 迁移到全天候宿主（推荐复用已部署的
   阿里云北京实例 `47.93.234.51`，同时消除时区错位），或提供等效的宿主常驻保障；
   ③ 迁移后以 1 个交易日重验：交易时段槽位成功率 ≥ 95%、无 >15 分钟空档、
   09:25–09:45 开盘窗口至少 1 次成功轮询。三项全过方可解锁 P4.3 生产触发。
