...
- [ ] 01 总览：风格堆叠图/全行业热力/环比箭头/通知铃铛
- [ ] 03 选股：全市场筛选、胜率列、真实因子权重、状态 diff、导出
- [ ] 04 个股：完整行情头、多周期K线+B/S、五维雷达、AI解读、事件tabs、目标区间
- [ ] 05 自选：自动漂移摘要、事件图标与事件流、分组管理、市值 donut
- [ ] 06 板块：周期预测+胜率、生命周期轮、超买/反转、资金流 donut、联动榜
- [ ] 07 大盘：情绪指数、全市场宽度/涨跌停/量能环比、分时、AI监测、跨市场
- [ ] 08 提醒：目标区间/建议金额、模拟执行三态流水
- [ ] 09 复盘：组合归因四卡、净值对比、机会/错误表、事件时间线、AI建议
- [ ] 系统：通知中心、job 审计、无 LLM/无富途双降级、REAL 拦截回归
